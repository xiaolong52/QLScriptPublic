 ...
 */
 ...
 const debug = 0
 //---------------------------------------------------------------------------------------------------------
 let ckStr = ($.isNode() ? process.env.demo_data : $.getdata('demo_data')) || '';
 let msg, ck;
 let ck_status = true;
 let host = 'xxx.xxxx.com';
 ...
 !(async () => {                  
	 let ckArr = await checkEnv(ckStr, "demo_data");
	 console.log("开始获取公告")
 ...
 async function signin() {
	 try {
		 ...
		 console.log(result);
		 if (result?.code == 0) {
			 DoubleLog(`签到:${result.data.list.infos.title} 🎉`);
			 await wait(3);
