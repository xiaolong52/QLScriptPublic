 ...
 *  项目昵称:xxx
 *
 * cron 0 0,7 * * *  demo.js         //拉库时定时 0 0,7 * * *     脚本文件名demo.js  拉库时会自动填写
 *
 * 9-5		签到兑换实物
 *
 * ========= 青龙--配置文件 =========
 * 变量格式: export demo_data=''   ,多账号用 换行 或 @ 分割
 ...
 */
 ...
 //---------------------------------------------------------------------------------------------------------
 let ckStr = ($.isNode() ? process.env.demo_data : $.getdata('demo_data')) || '';    //检测青龙是否存在变量demo_data//改变量名只需要改下这个还有下面的demo_data
 ...
 async function signin() {      //signin就是签到的意思 自己改
	 try {
		 let url = {
			 url: `${hostname}/xxx/xxx`,     //刚才提到了  hostname
			 headers: {
				 "Host": host,         //刚才提到的host
				 "user-agent": "xxx",      //UA  
				 "cookie": ck[0]          //Cookie    UA  COOKIE都要根据你抓包所必需的请求 来写 可以往下继续写 但是写完必须加逗号     ck[0]刚才也提到了
			 },
		 ...
		 if (result?.code == 0) {                                   //如果返回值中的code等于0 则
			 DoubleLog(`签到:${result.data.list.infos.title} 🎉`);        //输出 json结构树  这个没办法演示 自己感觉吧  
			 await wait(3);
		 } else if (result?.code == 1) {
			 DoubleLog(`签到:${result.msg},请勿重复签到`);          //多个状态码多个输出
		 } else {
			 DoubleLog(`签到: 失败 ❌ 了呢,原因未知!`);         //否则就报错
			 console.log(result);                //输出返回
		 }
	 } catch (error) {
		 console.log(error);
