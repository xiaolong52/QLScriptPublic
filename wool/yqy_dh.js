 ...
 *  项目昵称:优企盈 兑换
 *
 * cron 0 0,7 * * *  yqy_dh.js         //拉库时定时 0 0,7 * * *     
 * 10-18		检测奖品列表 满足条件且兑换
 *
 * ========= 青龙--配置文件 =========
 * 变量格式: export yqy_dh_data=''   ,多账号用 换行 或 @ 分割
 * 抓域名 下请求中 xxxxxx 的值填入变量
 */

 const { log } = require("console");

 const $ = new Env("优企盈 兑换");
 const notify = $.isNode() ? require("./sendNotify") : "";
 const Notify = 1
 const debug = 0
 //---------------------------------------------------------------------------------------------------------
 let ckStr = ($.isNode() ? process.env.yqy_dh_data : $.getdata('yqy_dh_data')) || '';
 let msg, ck;
 let ck_status = true;
 ...
 !(async () => {                  
	 let ckArr = await checkEnv(ckStr, "yqy_dh_data");
	 console.log("开始获取公告")
 ...
 /**
  * 获取积分信息    httpGet或者httpPost  看你的请求头
  */
 async function getinfo() {
	 try {
		 ...
		 //console.log(result);
		 if (result?.code == 0) {
			 DoubleLog(`该用户ID为:${result.data.userId} 🎉积分查询:${result.data.integralNum} `);
			 await wait(3);
		 } else {
   ...
   async function getlist() {
	try {
		...
		if (result?.code == 0) {
			...
			//DoubleLog(`查询奖品ID:${result.data[index].prizeId} 🎉,奖品昵称为:${result.data[index].prizeName} `);
  ...
  */
  async function exchange() {
    try {
        ...
        //console.log(result);
        if (result?.code == 0) {
            DoubleLog(`兑换奖品ID${ck[2]}成功:${result.message} 🎉 `);
            await wait(3);
        } else {
