 ...
 *  项目昵称:优企盈
 *
 * cron 0 0,7 * * *  yqy.js         //拉库时定时 0 0,7 * * *     
 *
 * 9-5		积分兑换实物
 *
 * ========= 青龙--配置文件 =========
 * 变量格式: export yqy_data=''   ,多账号用 换行 或 @ 分割
 * 账号密码 用&连接
 */
 ...
 const debug = 0
 //---------------------------------------------------------------------------------------------------------
 let ckStr = ($.isNode() ? process.env.yqy_data : $.getdata('yqy_data')) || '';
 let msg, ck;
 ...
 let textArr = ['怎样制止对家具的破坏，养成宠物的好性格和好习惯。','换毛期怎么做卫生','选择适合狗狗玩的玩具 如何跟狗狗培养默契',]
 ram_num = randomInt(0, 2)
 let text = textArr[ram_num];
 ram_num1 = randomInt(0, 3)
 let text1Arr = ['社会中的政治和经济密不可分，在职场中职场政治和个人能力同样密不可分，职场的精英们个个有能力，懂政治。','个人能力表现为时间掌控能力、知识水平、现场问题解决能力，职场政治能力表现为判断自身所处环境的能力以及创造利于自己条件的能力。','真正的职场精英是可以做到三件事的：我知道该做什么，我知道该怎么做，我有时间去做。','即我有时间去的事情，我才知道该怎么做，然后我才能知道这件事情该不该做。']
 let text1 = text1Arr[ram_num];
 //---------------------------------------------------------------------------------------------------------
 ...
 !(async () => {                  
	 let ckArr = await checkEnv(ckStr, "yqy_data");
	 console.log("开始获取公告")
 ...
 async function start() {
  ...
  async function auth() {
    try {
        ...
        let result = await httpPost(url, `账号登陆`);

        //console.log(result);
        if (result?.code == 0) {
            DoubleLog(`用户ID:${result.data.userId}🎉 用户token为: ${result.data.token}🎉`);
            await wait(3);
            user_id=result.data.userId
			user_token=result.data.token
        } else {
            DoubleLog(`登录用户信息: 失败 ❌ 了呢,原因未知!`);
           // console.log(result);
        }
    } catch (error) {
        console.log(error);
 ...
 async function userinfo() {
    try {
        let url = {
            url: `${hostname}/v2/users/basic_info`,
            headers: {
                "Host": host,
                "token": user_token
        ...
        //console.log(result);
        if (result?.code == 0) {
            DoubleLog(`用户ID:${result.data.userId} 用户昵称${result.data.nickname}🎉`);
            await wait(3);
            user_id=result.data.userId
        } else {
 ...
 async function prizelist() {
    try {
        ...
        let result = await httpGet(url, `奖品列表查询`);

        //console.log(result);
        if (result?.code == 0) {
            for (var index =0; index < result.data.length;index++) {  //输出所有任务
            DoubleLog(`奖品ID:${result.data[index].prizeId} 奖品名称${result.data[index].prizeName} 兑换该奖品所需要的积分${result.data[index].integralNum}🎉`);
            }
            await wait(3);
        } else {
            DoubleLog(`查询奖品信息: 失败 ❌ 了呢,原因未知!可能是因为第一个账号失效了呢！`);
            //console.log(result);
        }
    } catch (error) {
        console.log(error);
 ...
 async function tasklist() {
    try {
        let url = {
            url: `${hostname}/v2/integral/task/list`,
            headers: {
                "Host": host,
                "token": user_token
            },
        ...
        let result = await httpGet(url, `任务列表查询`);

       // console.log(result);
        if (result?.code == 0) {
            for (var index =0; index < result.data.length;index++) {  //输出所有任务
            DoubleLog(`任务ID:${result.data[index].taskId} 任务名称${result.data[index].taskName} 任务所获得的积分${result.data[index].integralNum}🎉`);
            }
            await wait(3);
        } else {
            DoubleLog(`查询任务信息: 失败 ❌ 了呢,原因未知!`);
            //console.log(result);
        }
    } catch (error) {
        console.log(error);
 ...
 async function signin() {
	 try {
		 ...
		 if (result?.code == 0) {
			 ...
			 DoubleLog(`签到: 失败 ❌ 了呢,原因${result.message}!`);
			 //console.log(result);
		 }
	 } catch (error) {
		 console.log(error);
   ...
   async function information_list() {
	try {
		...
		if (result?.code == 0) {
			...
			DoubleLog(`获取文章成功:${result.data[index].informationId} 🎉`);
			//await wait(3);
			information_idnum=result.data[index].informationId
			await information_read(information_idnum);}
			information_idnum=result.data[0].informationId
  ...
  */
   async function information_read(information_idnum) {
	try {
		...
		//console.log(result);
		if (result?.code == 0) {
			DoubleLog(`阅读文章成功:${result.data.informationId} 🎉`);
			await wait(3);
		} else {
			DoubleLog(`获取: 失败 ❌ 了呢,原因未知!`);
			//console.log(result);
  ...
  */
   async function postingpush() {
	try {
		...
		//console.log(result);
		if (result?.code == 0) {
			DoubleLog(`发布文章成功:${result.message} 🎉`);
			await wait(3);
		} else {
			DoubleLog(`获取: 失败 ❌ 了呢,原因未知!`);
			//console.log(result);
