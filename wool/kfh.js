 ...
 *  项目昵称:卡夫亨
 *
 * cron 0 0,7 * * *  kfh.js         //拉库时定时 0 0,7 * * *     
 * 9-5		签到兑换实物
 *
 * ========= 青龙--配置文件 =========
 * 变量格式: export kfh_data=''   ,多账号用 换行 或 @ 分割
 * 抓域名 https://fscrm.kraftheinz.net.cn/crm/public 下请求中 token 的值填入变量
 */
 ...
 const debug = 0
 //---------------------------------------------------------------------------------------------------------
 let ckStr = ($.isNode() ? process.env.kfh_data : $.getdata('kfh_data')) || '';
 let msg, ck;
 ...
 !(async () => {                  
	 ...
	 for (let index = 0; index < ckArr.length; index++) {
		 ...
		 await start();
		 datalength = ckArr.length
		 data=ckArr
		 if (index == datalength-1) {  //当账号执行到最后一个时 开始执行互助
			console.log("开始输出助力池");
			console.log(inviteId)
			recordshare(datalength,data)
		 }                            //单独拿出来一个账号执行 strat函数任务
  ...
  */  //循环助力 第二个账号助力第一个账号的链接 依次类推 最后一个账号被账号一助力
  async function recordshare(datalength,data) {
	...
	try {
		...
		//console.log(result);
		if (result?.error_code == 0) {
			DoubleLog(`内部互助第${index+1}个账号被助力成功:${result.msg} 🎉`);
			await wait(3);
		} else {
			DoubleLog(`内部互助: 失败 ❌ 了呢,原因未知!`);
			//console.log(result);
		}
		if(index == datalength-2){//最后一个账号
			inviteIdARRlength=inviteId.length-1  //数组长度减去1就是数组最后一位
			try {
				let url = {
					url: `${hostname}/crm/public/index.php/api/v1/recordScoreShare`,
					headers: {
					...
					body:inviteId[inviteIdARRlength]
				};
				let result = await httpPost(url, `内部互助`);
		
				//console.log(result);
				if (result?.error_code == 0) {
					DoubleLog(`内部互助最后一个账号被助力成功:${result.msg} 🎉`);
					await wait(3);
				} else {
					DoubleLog(`内部互助: 失败 ❌ 了呢,原因未知!`);
					console.log(result);
		 ...
		 $.get(
			 ...
			 async (err, resp, data) => {
				 try {
					 ...
					 //-----------console.log(err, resp);  这里会报错网站的 信息  默认开启
					 console.log(`\n ${tip} 失败了!请稍后尝试!!`);
					 msg = `\n ${tip} 失败了!请稍后尝试!!`
				 } finally {
					 resolve();
