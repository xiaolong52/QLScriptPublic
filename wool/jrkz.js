 ...
 * 
 * 多账号用 换行 或 @ 分割
 * 请在 api.jrkz66.com headers中找到Cookie => qianfan_device 的值 小黄鸟 放大镜搜索即可找到
 * 
 * 变量格式 用户名&密码&qianfan_device
 * export jrkz_data='sxxxxx...&x121212...&c45132a...'
 * ====================================
 *   
 */
...
class UserInfo {
    constructor(str) {
        ...
        this.device = utils.base64_encode(str.split('&')[2]);
        this.username = str.split('&')[0];
        this.userpassword = utils.base64_encode(str.split('&')[1])
        //let ck = str.split('&')
        //this.data1 = ck[0]
        ...
        this.randomtime = utils.randomInt(15, 61)
    }

    async user_login1(name) { // 初始化token
        let codeSign = utils.MD5_Encrypt(this.nonce + this.key + this.ts).toUpperCase()
        try {
            let options = {
                method: 'GET',
                url: 'http://api.jrkz66.com/v5_0/user/get-token',
                headers: {
            ...
            //console.log(result);
            if (result.ret == 0) {
                //DoubleLog(`账号[${this.index}] 获取初始化token: ${result.data.third_app_token} `)
                return this.ck1 = result.data.third_app_token
            } else {
    ...
    async user_login2() { // 获取用户Bearer
        let appToken = await this.user_login1("获取token");
        let body = {
            encode: 1,
        ...
        try {
            let options = {
                method: 'POST',
                url: 'http://api.jrkz66.com/v5_0/user/login',
                headers: {
            ...
            //console.log(result);
            if (result.ret == 0) {
                //DoubleLog(`账号[${this.index}] 获取Bearer: ${result.data.bearerToken} `)
                this.ck2 = result.data.bearerToken
            } else {
                DoubleLog(`账号[${this.index}] 获取:失败 ❌ 了呢,原因未知！`);
                console.log(result);
            }
