 ...
 * 长安深蓝  APP
 *
 * cron 25 5,7,9,11,13 * * *  casl.js
 *
 * 9-12    积分兑换实物
 * 作者:小鹿 - QQ860562056 麻烦点个star给仓库
 * 测试:你李貌吗 - 漫步者 
 * ========= 青龙--配置文件 =========
 ...
 */
 ...
 async function user() {
	try {
		let url = {
			url: `${hostname}/appapi/v1/member/ms/wallet`,
			headers: {
				"Host": host,
				"authorization": ck[0],
				"appType": "android"
		...
		let result = await httpGet(url, `个人信息查询`);

		//console.log(result);
		if (result?.code == 200) {
			DoubleLog(`当前用户:${result.data.memberName} 🎉当前用户积分为:${result.data.countPoint}`);
			await wait(3);
		} else {
 ...
 /**
  * tasklist    httpGet  任务列表获取
  */
 async function tasklist() {
	 try {
		 let url = {
			 url: `${hostname}/appapi/v1/member/member/ms/point/task`,
			 headers: {
				 "Host": host,
				 "authorization": ck[0],
				 "appType": "android"
		 ...
		 let result = await httpGet(url, `任务列表获取`);
 
		 //console.log(result);
		 if (result?.code == 200) {
			for (var listnum = 0; listnum < result.data.length; listnum++){
			 DoubleLog(`获取到任务: ${result.data[listnum].ruleName} 🎉`);
			 await wait(2);}
		 } else {
			 DoubleLog(`获取任务 失败 ❌ 了呢,原因未知!`);
			 console.log(result);
		 }
		for (var listnum1 = 0; listnum1 < result.data.length; listnum1++){
			if (result.data[listnum1].finiseNum < result.data[listnum1].ruleUpperLimit) {
				//console.log("判断到" + `${result.data[listnum1].ruleName}` + "未做完，开始尝试做该任务");
				//这里开始做任务或者后加做任务
				//console.log(`${result.data[listnum1].ruleCode}`);
				// if (result.data[listnum1].ruleCode === "BC00009") {
				// 	console.log("现在要开始执行签到啦~~~~");
				// 	// await dianzan;
				// } else if (result.data[listnum1].ruleCode === "BC00001") {
				// 	console.log("现在开始执行浏览啦~~~");
				// } else if (result.data[listnum1].ruleCode === "BS00001") {
				// 	console.log("现在开始执行评论啦~~~");
				// } else if (result.data[listnum1].ruleCode === "BC00001") {
				// 	console.log();
				// } else if (result.data[listnum1].ruleCode === "BC00001") {
				// 	console.log();
				...
				// 	console.log("结束");
				// }
				switch (result.data[listnum1].ruleCode) {
					case "BC00009":
					console.log("开始执行签到");
					break;
					//await qiandao;
					...
					break;
					case "BS00006":
					console.log("开始执行大家发帖");
					break;
					case "BS00007":
					console.log("开始执行精选发帖");
					break;
					case "BC00004":
					console.log("开始执行浏览活动");
					break;
					case "BS00013":
					console.log("开始执行活动晒图");
					break;
					case "BS00003":
					console.log("开始执行活动分享");
					break;
				}

			} else {
				console.log("任务 "+ `${result.data[listnum1].ruleName}` + "完啦！");
			}
